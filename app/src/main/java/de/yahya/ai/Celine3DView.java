...
/** Realtime 3D Celin renderer. The final rigged avatar lives at assets/models/celine.glb. */
public final class Celine3DView extends FrameLayout implements Choreographer.FrameCallback {
    ...
    private float yaw, pitch, speech;
    ...
    private void initializeRenderer() {
        try {
            ...
            // Kotlin exposes the SurfaceView constructor to Java as the full four-argument
            // overload: (SurfaceView, Engine, UiHelper, Manipulator). Do not fill synthetic
            // constructor parameters with null; that was fragile and could not compile/run
            // consistently across Filament releases.
            Class<?> manipulatorClass = Class.forName("com.google.android.filament.utils.Manipulator");
            Constructor<?> ctor = viewerClass.getConstructor(SurfaceView.class, Engine.class, UiHelper.class, manipulatorClass);
            viewer = ctor.newInstance(surface, engine, helper, null);
            ...
            Skybox skybox = new Skybox.Builder().color(0.018f,0.018f,0.028f,1f).build(engine);
    ...
    private ByteBuffer readAsset(String path) throws Exception {
        ...
        in.close(); return ByteBuffer.wrap(out.toByteArray());
    }
    ...
    public void setGaze(float x,float y){yaw=clamp(x,-1f,1f)*7f;pitch=clamp(y,-1f,1f)*4f;}
    public void releaseGaze(){yaw*=.35f;pitch*=.35f;}
    ...
    private void animateRoot(long now){
        ...
        float breathe=(float)Math.sin(t*1.45f)*.35f;
        float statePitch=0f,stateYaw=0f,z=0f;
        switch(state){
            case LISTENING: statePitch=-1.2f+(float)Math.sin(t*1.2f)*.45f;z=.012f;break;
            case THINKING: stateYaw=(float)Math.sin(t*.65f)*2f;statePitch=1f;break;
            case SPEAKING: stateYaw=(float)Math.sin(t*1.8f)*(.8f+speech*1.2f);statePitch=(float)Math.sin(t*2.35f)*(.45f+speech*.9f);z=speech*.010f;break;
            default: stateYaw=(float)Math.sin(t*.38f)*.55f;break;
        }
        float rx=(pitch+statePitch+breathe*.16f)*.0174532925f, ry=(yaw+stateYaw)*.0174532925f;
        float sx=(float)Math.sin(rx),cx=(float)Math.cos(rx),sy=(float)Math.sin(ry),cy=(float)Math.cos(ry);
        float[] m=new float[]{cy,sx*sy,-cx*sy,0, 0,cx,sx,0, sy,-sx*cy,cx*cy,0, 0,breathe*.0018f,z,1};
        try{
            int root=(Integer)asset.getClass().getMethod("getRoot").invoke(asset);
            TransformManager tm=engine.getTransformManager(); int instance=tm.getInstance(root);
            if(instance!=0)tm.setTransform(instance,m);
        }catch(Throwable ignored){}
    }

    public void destroy(){
        running=false;choreographer.removeFrameCallback(this);
        if(viewer!=null&&destroyModelMethod!=null)try{destroyModelMethod.invoke(viewer);}catch(Throwable ignored){}
        viewer=null;asset=null;
    }

    private static String rootName(Throwable t){Throwable x=t;while(x.getCause()!=null)x=x.getCause();return x.getClass().getSimpleName();}
