...
public class MainActivity extends Activity implements TextToSpeech.OnInitListener {
    ...
    private TextToSpeech tts; private boolean ttsReady=false; private int ttsInitTries=0; private MediaPlayer neuralPlayer; private LocalNeuralTtsEngine localNeuralTts; private SupertonicModelManager supertonicModels; private CelineAvatarController avatarController; private SharedPreferences prefs; private CelineStructuredMemory memory; private CelineGoalTaskRuntime goalTaskRuntime; private CelineContextBrokerG14 contextBroker; private DeviceBridge device; private Handler handler=new Handler(Looper.getMainLooper());
    ...
    @Override protected void onCreate(Bundle b){super.onCreate(b);prefs=getSharedPreferences("yahya_ai",MODE_PRIVATE);memory=new CelineStructuredMemory(prefs);goalTaskRuntime=new CelineGoalTaskRuntime(prefs);contextBroker=new CelineContextBrokerG14(memory,goalTaskRuntime.graph(),goalTaskRuntime.graph());device=new DeviceBridge(this);localNeuralTts=new LocalNeuralTtsEngine(this);supertonicModels=new SupertonicModelManager(this);setContentView(buildUi());initTts();addAssistant("Hallo Yahya. Schön, dass du da bist. Was machen wir?",false);if(prefs.getBoolean("wake",false)){Intent ws=new Intent(this,WakeWordService.class);if(Build.VERSION.SDK_INT>=26)startForegroundService(ws);else startService(ws);}handleWakeIntent(getIntent());}
