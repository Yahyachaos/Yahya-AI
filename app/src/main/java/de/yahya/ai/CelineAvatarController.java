...
public final class CelineAvatarController implements SpeechAudioBus.Listener {
    ...
    public void lookToward(float nx,float ny){userLooking=true;if(motionView==null)return;float x=clamp(nx,-.5f,.5f),y=clamp(ny,-.5f,.5f);motionView.animate().cancel();motionView.setTranslationX(x*dp(4.5f));motionView.setTranslationY(y*dp(2.4f));motionView.setRotation(x*.55f);if(face!=null)face.setGaze(x*2f,y*2f);}
    public void releaseLook(){userLooking=false;if(motionView!=null)motionView.animate().translationX(0).translationY(0).rotation(0).scaleX(1).scaleY(1).setDuration(500).setInterpolator(new AccelerateDecelerateInterpolator()).start();if(face!=null)face.releaseGaze();}
    ...
    private void playNaturalGaze(){if(face==null)return;float rx=state==State.THINKING?.50f:.27f,ry=state==State.THINKING?.31f:.18f;float gx=r(-rx,rx),gy=r(-ry,ry);if(random.nextInt(5)!=0){gx*=.42f;gy*=.42f;}face.setGaze(gx,gy);handler.postDelayed(()->{if(!released&&!userLooking&&face!=null)face.releaseGaze();},450+random.nextInt(750));}
    ...
    private void stopLoopsOnly(){try{if(breath!=null)breath.cancel();}catch(Exception ignored){}try{if(lift!=null)lift.cancel();}catch(Exception ignored){}breath=null;lift=null;speechEnergy=0;gestureRunning=false;if(motionView!=null){motionView.animate().cancel();motionView.setScaleX(1);motionView.setScaleY(1);motionView.setRotation(0);motionView.setTranslationX(0);motionView.setTranslationY(0);}if(avatar!=null){avatar.setTranslationX(0);avatar.setTranslationY(0);}}
