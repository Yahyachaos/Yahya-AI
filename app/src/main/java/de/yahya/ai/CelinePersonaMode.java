...
/**
 * App-owned deterministic persona-mode state for Celine.
 *
 * <p>This class deliberately owns only persona style state. It has no tool,
 * permission, memory, room/avatar, network, or persistence authority.</p>
 */
public final class CelinePersonaMode {
    ...
    /**
     * Applies only deterministic direct-user control commands. Non-user channels
     * are always ignored so model/tool text cannot activate the mode.
     */
    public synchronized CommandResult apply(InputChannel channel, String input) {
        ...
        if (isAsciiInteger(suffix)) {
            // Explicit but out-of-range numeric intensity: consume the control
            // command, flag it, and preserve the last valid persona state.
            return result(true, true);
        }

        // Malformed text such as "Nutte stark" is ordinary conversation, not
        // a control command. Fail closed and preserve the current state.
        return result(false, false);
    }

    /**
     * Returns only a bounded style contribution. The contribution explicitly
     * preserves identity, permissions, privacy/memory policy, safety behavior,
     * and factual honesty.
     */
