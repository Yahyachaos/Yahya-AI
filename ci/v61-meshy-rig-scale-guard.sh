...
VERSION_CODE="$(sed -n 's/^[[:space:]]*versionCode[[:space:]]\+\([0-9][0-9]*\).*/\1/p' "$GRADLE" | head -n1)"
[[ "$VERSION_CODE" =~ ^[0-9]+$ ]] || { echo "ERROR: versionCode missing"; exit 1; }
(( VERSION_CODE >= 61 )) || { echo "ERROR: versionCode below protected v61 baseline: $VERSION_CODE"; exit 1; }
...
echo "v61 Meshy rig-scale guard OK for versionCode $VERSION_CODE"
