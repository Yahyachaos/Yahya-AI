...
case "$scope" in
  9r5-shelf)
    exec timeout 720s bash ci/celine-9r5-shelf-proof.sh "$apk" "$out"
    ;;
  9r5-mirror)
    exec timeout 720s bash ci/celine-9r5-mirror-proof.sh "$apk" "$out"
    ;;
  9r5-dresser)
    exec timeout 720s bash ci/celine-9r5-dresser-proof.sh "$apk" "$out"
    ;;
  9r5-window)
    exec timeout 720s bash ci/celine-9r5-window-proof.sh "$apk" "$out"
    ;;
  9r4)
