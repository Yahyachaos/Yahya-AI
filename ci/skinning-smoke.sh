...
fail_skin() {
  ...
  adb logcat -d | grep -E 'de\.yahya\.ai|Filament|gltfio|FATAL EXCEPTION|SIGABRT|V54-|V55-|V56-|V57-|REN-' | tail -300 || true
  exit 1
}
...
[[ -n "$PID" ]] || fail_skin "Yahya AI process died before v57 CALL proof"

adb shell uiautomator dump /sdcard/v57-home.xml >/dev/null || fail_skin "Could not dump HOME UI"
adb pull /sdcard/v57-home.xml v57-home.xml >/dev/null || fail_skin "Could not pull HOME UI"
read -r TAP_X TAP_Y <<< "$(python3 - <<'PY'
import re, xml.etree.ElementTree as ET
root=ET.parse('v57-home.xml').getroot()
...
sleep 6
adb shell uiautomator dump /sdcard/v57-call.xml >/dev/null || fail_skin "Could not dump CALL UI"
adb pull /sdcard/v57-call.xml v57-call.xml >/dev/null || fail_skin "Could not pull CALL UI"
grep -q 'Live mit Celin' v57-call.xml || fail_skin "CALL did not open for v57 proof"
...
grep -q 'V57-120' emulator-celine-diagnostics.xml || fail_skin "V57-120 head/gaze refinement marker missing"
...
if grep -q 'V57-198\|V57-199\|V56-198\|V56-199\|V55-198\|V55-199\|V54-198\|V54-199' emulator-celine-diagnostics.xml; then
...
[[ -n "$PID_AFTER" ]] || fail_skin "Process died restoring HOME after v57 proof"
echo "v57 CALL head/gaze refinement proof passed with PID=$PID_AFTER"
echo "Verified: v56 Hips+neck+Head skinning + V57 marker + moving pixels + HOME recovery"
